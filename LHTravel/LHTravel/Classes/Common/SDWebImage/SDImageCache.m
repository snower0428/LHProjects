...
static natural_t get_free_memory(void)
{
    ...
    {
        MLOG(@"Failed to fetch vm statistics");
        return 0;
    }
...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        */
        diskCachePath = SDWIReturnRetained(kMsgImagePath);
...
{
    if (![[NSFileManager defaultManager] fileExistsAtPath:kMsgImagePath])
    {
        [[NSFileManager defaultManager] createDirectoryAtPath:kMsgImagePath
                                  withIntermediateDirectories:YES
                                                   attributes:nil
                                                        error:NULL];
    }
...
- (void)queryDiskCacheOperation:(NSDictionary *)arguments
{
    ...
    if (image)
    {
        UIImage *decodedImage = image;//[UIImage decodedImageWithImage:image];
        if (decodedImage)
        {
            image = decodedImage;
        }
...
//added by ygf
- (NSString *)imageFilePathFromKey:(NSString *)key
{
    if (key == nil)
    {
        return nil;
