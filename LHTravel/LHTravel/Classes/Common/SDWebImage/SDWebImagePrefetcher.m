...
- (void)reportStatus
{
    ...
    MLOG(@"Finished prefetching (%d successful, %d skipped, timeElasped %.2f)", total - _skippedCount, _skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
#endif
}
...
{
    _finishedCount++;
    MLOG(@"Prefetched %d out of %d", (int)_finishedCount, (int)[self.prefetchURLs count]);
...
{
    _finishedCount++;
    MLOG(@"Prefetched %d out of %d (Failed)", (int)_finishedCount, (int)[self.prefetchURLs count]);
