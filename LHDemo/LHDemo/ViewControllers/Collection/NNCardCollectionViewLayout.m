...
- (CGSize)collectionViewContentSize
{
    ...
    return CGSizeMake(self.collectionView.bounds.size.width, /*self.collectionView.bounds.size.height * count*/1000*count);
}
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForDecorationViewOfKind:(NSString *)decorationViewKind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (offsetY > self.collectionView.contentOffset.y) {
		float y = 1.0f - (offsetY - self.collectionView.contentOffset.y) / 1000;//self.collectionView.frame.size.height;
        NSLog(@"%f",y);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
	...
	CGFloat offsetY = indexPath.item * 1000;//self.collectionView.bounds.size.height;
    CGRect frame;
    frame.origin.x = 0;
    frame.origin.y = offsetY;
	frame.size = CGSizeMake(self.collectionView.bounds.size.width, 1000);//self.collectionView.bounds.size;
    attr.frame = frame;
...
{
	CGFloat cellHeight = 1000;//self.collectionView.bounds.size.height;
